...
public class MaterialPowerRule implements Rule {
    ...
    public RuleResult apply(String imageId, RuleContext context) {
        ...
        for(BranchInfo branch : branches){
            ...
            if(!cableType.equals(targetCableType)) return RuleResult.fail("电缆型号不在清单中");

            //电缆规格不得小于3*2.5mm²
            int cableSize=branch.getCable_size();
            if(cableSize<3*2.5) return RuleResult.fail("电缆规格不得小于3*2.5mm²");
            ...
            if(mainCableSize<branchCableSize) return RuleResult.fail("主电缆规格不得小于分电缆规格");
            ...
            if(capacity != number*power) return RuleResult.fail("容量不等于数量*功率");
